...
支持任何兼容 OpenAI 格式的接口。设置页内置了几家常见服务商的预设，也可以手动添加任意自定义端点，包括自部署或公司内部网关。

> ⚠️ 描述中**不要列举服务商品牌名**。v0.3.0 首次提交因这一句里罗列了多家模型服务商名称，被判 Keyword Spam（violation ref: Yellow Argon）而拒。品牌名放在设置页 UI 里即可，商店描述只说「兼容 OpenAI 格式的任意接口」。
