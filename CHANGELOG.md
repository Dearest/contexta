...
- **三下空格润色**：在任意输入框连按三下空格，把中英混写的内容改写成地道英文。只补全你写不出的部分（中文→英文），不重写你已经写好的英文；浮层中缺口（绿色、较大）与纠错（灰色、较小）分开展示。替换走 `execCommand('insertText')`，保留原生撤销栈，⌘Z 即可还原。输入法选词时的空格不会误触发。
- 润色使用独立的「润色模型」，未配置时依次回退到划词模型、翻译模型。
- **英语表达缺口记录**（默认关闭）：把润色中的缺口与纠错追加到 Obsidian 的一篇笔记；未配置 Obsidian 时存在本地（最多 500 条）。设置页可开关并指定笔记路径。
- **X（Twitter）精准翻译**：只翻译推文正文、评论和 X 长文（Articles），不再翻译昵称、@handle、时间、按钮文字和回复框占位符。翻译后滚动加载的新推文会自动翻译；X 回收后重新出现的推文直接复用已有译文，不重复调用模型。离开当前页面即停止。

### Fixed

- 划词翻译弹窗在「选中文字后滚动页面再点绿点」时会偏离数百像素。

### Changed

- 隐私政策补充润色功能发送与存储的数据说明。
